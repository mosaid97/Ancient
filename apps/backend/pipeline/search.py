...
WHERE ($tier_filter IS NULL OR chunk.documentId IN (
    SELECT d.id FROM (
        MATCH (d:DOCUMENT {tier: $tier_filter}) RETURN d.id
    )
))
RETURN
...
# Simpler version — tier/language filtering done post-fetch in Python
